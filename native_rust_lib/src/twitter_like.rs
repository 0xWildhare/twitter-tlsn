// modified from TLSN/tlsn/examples/twitter
...
async fn main() {
    ...
    dotenv::dotenv().ok();
    let tweet_id = env::var("TWEET_ID").unwrap();
    ...
    let request = Request::builder()
        .uri(format!("https://{SERVER_DOMAIN}/{ROUTE}/{tweet_id}.json"))
    ...
    let mut file = tokio::fs::File::create("twitter_like.json").await.unwrap();
    file.write_all(
    ...
    let mut file = tokio::fs::File::create("twitter_like_proof.json")
        .await
        .unwrap();
